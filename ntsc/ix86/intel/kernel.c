...
void k_activate(dcb_ro_t *rop, uint32_t reason) 
{
    ...
    INFO_PAGE.pvsptr = &rwp->pvs; // info page is updated on domain switch!!
...
#define CMDLINE_OFFSET (*(unsigned short *) (PARAM+0x22))
	...
	*(uint32_t *)&p->vendor[8]=out[2];
    ...
    bad_cx.gpreg[r_eax] = (uint32_t)primal; //nexus pointer
    bad_cx.gpreg[r_ebx] = (int) (&k_st); /* XXX- urgh */ // kernel state
