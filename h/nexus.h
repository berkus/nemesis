...
struct nexus_primal {
    ...
    size_t		lastaddr;      /* address of first free page after Nemesis image. Same as nexus_EOI record. */
};
...
struct nexus_name {
    enum nexus_info	tag; /* nexus_namespace */
    size_t		naddr; /* pointer to namespace_entry array */
    size_t		nsize;
    int			refs;
    int			nmods;
